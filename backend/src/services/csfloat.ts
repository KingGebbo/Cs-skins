...
// Rate-limited client: max 10 requests per second
...
// Simple request queue to avoid hammering the API
let lastRequestTime = 0;
const MIN_REQUEST_INTERVAL_MS = 200; // max ~5 req/s
...
export async function fetchMarketStats(
  marketHashName: string
): Promise<CSFloatMarketItem | null> {
  try {
    const data = await rateLimitedGet<CSFloatMarketItem>('/market/stats', {
      market_hash_name: marketHashName,
    });
    return data;
  } catch {
...
// Generate synthetic Eastern market data (BAF163/YuPin) using a volume multiplier
// In production this would call the actual marketplace APIs
export function estimateEasternMarketData(
  westernVolume: number,
  westernPrice: number
): { volume: number; price: number } {
  // Eastern markets typically run 1.1–1.4x western volume during normal times
  // During pump phases the multiplier becomes 2–5x
  const baseMultiplier = 1.1 + Math.random() * 0.3;
  return {
    volume: Math.round(westernVolume * baseMultiplier),
    price: westernPrice * (0.85 + Math.random() * 0.15), // CNY-adjusted price
  };
}
